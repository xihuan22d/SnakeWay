...
import com.snakeway.network.utils.TimeTool;
import com.snakeway.ui.utils.ActivityManager;
import com.snakeway.ui.utils.SnakeWayManager;
import com.snakeway.ui.utils.SnakeWayManagerParams;
import com.snakeway.ui.utils.ToastCompat;
...
/**
 * Created by snakeway on 2017/9/15.
 */
public class BaseActivity extends AppCompatActivity {
    ...
    public SnakeWayManagerParams getSnakeWayManagerParams() {
        return getDefaultSnakeWayManagerParams(this);
    }
    ...
    protected void onDestroy() {
        if (cancelRequest) {
            ...
            if (cancelType == CANCELREQUESTWITHTAG) {
                SnakeWayManager.getInstance(this).cancelRequestWithTag(activityKey, true);
            } else if (cancelType == CANCELREQUESTWITHTAGBYWAIT) {
                SnakeWayManager.getInstance(this).cancelRequestWithTagByWait(activityKey, true);//销毁前先取消网络请求,最后在ActivityManager移除baseActivity可以让代码更清晰
            }
        }
    ...
    public static SnakeWayManagerParams getDefaultSnakeWayManagerParams(Context context) {
        return new SnakeWayManagerParams(context, 4, 2, new HttpAgreement_Default(), null, null, 2, null, 52428800, ResponseHandler.RESPONSEHANDLERWITHMESSAGERECORD, true, 2);
    }
    ...
    public static int getRecyclerViewSpace_Default(Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            space = activity.getResources().getDimensionPixelSize(R.dimen.snakeway_recyclerView_space_default);
        } else {
            space = activity.getResources().getDimensionPixelSize(R.dimen.snakeway_recyclerView_space_beforeLollipop_default);
        }
        return space;
    }
     ...
     * return NotNull
     */
    public static SnakeWayManager getSnakeWayManager(Context context) {
        SnakeWayManager snakeWayManager = null;
        Activity activity = BaseActivity.getActivityByContext(context);
        if (activity != null && activity instanceof BaseActivity) {
            BaseActivity baseActivity = (BaseActivity) activity;
            snakeWayManager = SnakeWayManager.getInstance(baseActivity);
        } else {
            snakeWayManager = SnakeWayManager.getInstance();
            if (snakeWayManager == null) {
                snakeWayManager = SnakeWayManager.getInstanceBySnakeWayManagerParams(BaseActivity.getDefaultSnakeWayManagerParams(context));
            }
            FileTool.needShowAndWriteLogToSdcard(RequestManager.openDebug, RequestManager.logFileName, "BaseActivity_getSnakeWayManager:context异常", null, 1);
        }
